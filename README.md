...
### 部署到公网（腾讯云轻量应用服务器 · 国内首选 · 最稳）
> 优点：新用户 **38~99 元/年**、24 小时在线、国内打开快、**代码原样搬、不用改**、用户存档写服务器磁盘持久不丢。
> 缺点：需一次性**实名认证**（身份证，几分钟）+ 首次充值几元（这一步必须本人操作）。

**第一步：买服务器（在腾讯云后台点）**
1. 打开腾讯云活动页（搜「腾讯云轻量应用服务器 新用户」），用**微信扫码**登录。
2. 按提示完成**实名认证**（上传身份证，几分钟审核）。
3. 购买时按下表选：
   | 选项 | 选什么 |
   |------|--------|
   | 镜像 | **系统镜像 → Ubuntu 22.04** |
   | 配置 | **2 核 2G**（新用户首单 38~99 元/年） |
   | 地域 | 选**上海**或**广州**（离你近） |
   | 带宽 | 4M 足够 |
   | 登录方式 | 设置 **root 密码**（记好，后面要用） |
   | 防火墙 | 确认放行 **80** 端口（默认一般已放） |
4. 付款后进入「轻量应用服务器」控制台，记下服务器的**公网 IP**。

**第二步：把代码传上去并一键跑起来**
1. 在控制台点服务器 → **登录** → 打开**浏览器在线终端**（不用装 SSH 软件）。
2. 在终端里，先把本机的 `english-master.zip` 通过终端的「上传」按钮传到 `/root` 目录。
   - 或用 Git（国内服务器连 GitHub 可能慢）：`git clone https://github.com/Xiaoyufeiyu/english-master.git`
3. 在终端执行（项目里已带 `deploy.sh`）：
   ```bash
   cd /root && bash deploy.sh
   ```
   脚本会自动装 Node、解压代码、用 pm2 启动在 **80 端口**、并设置开机自启，约 1 分钟完成。
4. 打开浏览器访问：**http://你的公网IP** （或腾讯云给的默认域名 `xxx.lightcp.com`）。
5. 用演示账号（韩立/王婵）或自己注册即可。

**运维命令**（需要时复制进终端）：
- 看日志：`pm2 logs english-master`
- 重启：`pm2 restart english-master`
- 停止：`pm2 stop english-master`

部署脚本见 `deploy.sh`，数据持久化在 `/root/english-master/data.json`。

---

### 部署到公网（Alwaysdata，海外备选 · 免绑卡 · 支持 qq 邮箱）
